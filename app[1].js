...
let currentView='today', selectedDay=null, timerInterval=null, installPrompt=null;
...
window.startTimer=(seconds,source)=>{clearInterval(timerInterval);let left=seconds;const original=source.textContent;source.classList.add('timer');source.textContent=fmtRest(left);timerInterval=setInterval(()=>{left--;source.textContent=fmtRest(Math.max(left,0));if(left<=0){clearInterval(timerInterval);source.textContent='REST DONE';navigator.vibrate?.([180,80,180]);setTimeout(()=>{source.textContent=original;source.classList.remove('timer')},1800)}},1000)};
